...
- Managed Hosts can store minimal SSH targeting settings. Private key values are referenced through Secrets and are
  materialized only by the Worker for Ansible execution. A Managed Host can optionally route through one Jump Host,
  modeled as another active Managed Host in the same customer.
